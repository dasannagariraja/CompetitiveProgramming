...
vector<Point>vec,vec1;
int intersect(){
  int i,j,n=vec.size(),m=vec1.size();
  for(i=0;i<vec.size();i++){
    for(j=0;j<vec1.size();j++){
      if(SegmentsIntersect(vec[i],vec[(i+1)%n],vec1[j],vec1[(j+1)%m])){
        return 1;
      }
    }
  }
...
int in(){
  int i,j,n=vec.size(),m=vec1.size();
  for(i=0;i<n;i++){
    if(PointInPolygon(vec1,vec[i])||PointOnPolygon(vec1,vec[i])){
      return 1;
    }
  }
  for(i=0;i<m;i++){
    if(PointInPolygon(vec,vec1[i])||PointOnPolygon(vec,vec1[i])){
      return 1;
    }
  }
  return 0;
